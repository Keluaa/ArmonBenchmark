...
@kernel function gpu_acoustic_kernel!(i_0, 
        ustar, pstar, @Const(rho), @Const(umat), @Const(pmat), @Const(cmat))
    i = @index(Global) + i_0
...
@kernel function gpu_acoustic_GAD_minmod_kernel!(i_0, 
        ustar, pstar, @Const(rho), @Const(umat), @Const(pmat), @Const(cmat), 
        @Const(ustar_1), @Const(pstar_1), dt, @Const(x))
    i = @index(Global) + i_0
    ...
    ustar[i] = ustar_1[i] + 0.5 * (1 - θ) * 
        (r_u_p * (umat[i] - ustar_1[i]) - r_u_m * (ustar_1[i] - umat[i-1]))
    pstar[i] = pstar_1[i] + 0.5 * (1 - θ) * 
...
@kernel function gpu_cell_update_lagrange_kernel!(i_0, ifin, dt, 
    ...
    # We must use x_ instead of X[i+1]-X[i] since X can be overwritten by other workgroups
...
@kernel function gpu_first_order_euler_remap_kernel!(i_0, dt, 
        @Const(X), @Const(ustar), @Const(rho), @Const(umat), @Const(Emat), 
        tmp_rho, tmp_urho, tmp_Erho)
    i = @index(Global) + i_0
...
@kernel function gpu_first_order_euler_remap_2_kernel!(i_0, 
        rho, umat, Emat, @Const(tmp_rho), @Const(tmp_urho), @Const(tmp_Erho))
    i = @index(Global) + i_0
...
function BizarriumEOS!(params, pmat, cmat, gmat, rho, emat)
    ...
    @simd_threaded_loop for i in ideb:ifin
        ...
        gmat[i] = 0.5/(rho[i]^3*cmat[i]^2)*(pk0second+(G0*rho0)^2*(pmat[i]-pk0))  # Formulae (8) + (11)
    end
end
...
function dtCFL(params, dta, x, cmat, umat)
    ...
    elseif params.use_gpu && use_ROCM
        ...
        result = zeros(eltype(x), 1)  # temporary array to hold the result of the reduction
